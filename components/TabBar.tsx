...
const TabBar: React.FC<TabBarProps> = ({ activeTab, onTabChange, isDarkMode }) => {
  ...
  return (
    <nav className={`flex-none h-[88px] ${isDarkMode ? 'bg-[#121212]/80 border-white/10' : 'bg-white/80 border-gray-200'} backdrop-blur-2xl border-t flex justify-around items-start pt-3 pb-8 px-2 select-none transition-colors duration-300`}>
      <button onClick={() => onTabChange(Tab.News)} className={getTabClass(Tab.News)}>
      ...
      <button onClick={() => onTabChange(Tab.Collection)} className={getTabClass(Tab.Collection)}>
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-6 h-6 mb-1">
      ...
      <button onClick={() => onTabChange(Tab.Bookmark)} className={getTabClass(Tab.Bookmark)}>
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-6 h-6 mb-1">
